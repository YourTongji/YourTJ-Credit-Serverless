...
async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const { title, description, rewardAmount, timestamp, nonce } = req.body as TaskCreateParams & { timestamp: number; nonce: string };
    ...
    await execute(
      `INSERT INTO tasks
      (task_id, creator_user_hash, title, description, reward_amount, status, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [taskId, userHash, title, description, rewardAmount, 'open', now]
    );
