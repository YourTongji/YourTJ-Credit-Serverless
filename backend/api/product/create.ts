...
async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const { title, description, price, stock, timestamp, nonce } = req.body as ProductCreateParams & { timestamp: number; nonce: string };
    ...
    await execute(
      ...
      (product_id, seller_user_hash, title, description, price, stock, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [productId, userHash, title, description, price, stock, 'available', now, now]
    );
