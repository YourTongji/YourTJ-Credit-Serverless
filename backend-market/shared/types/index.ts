...
export enum TaskStatus {
  ...
  SUBMITTED = 'submitted',     // 已提交（待发布者确认）
  COMPLETED = 'completed',     // 已完成
  CANCELLED = 'cancelled'      // 已取消
}
...
export interface Task {
  ...
  contactInfo?: string;      // 联系方式（仅对创建者/接单者可见）
...
export interface Product {
  ...
  deliveryInfo?: string;     // 发货信息/取货方式（仅买家/卖家可见）
  price: number;             // 商品价格
  stock: number;             // 库存数量
...
export interface Purchase {
  ...
  status: string;            // 状态
  createdAt: number;         // 创建时间戳
  acceptedAt?: number;       // 卖家接单时间戳
  deliveredAt?: number;      // 卖家标记交付时间戳
  confirmedAt?: number;      // 买家确认时间戳
  updatedAt?: number;        // 更新时间戳
  productTitle?: string;     // 商品标题（便于列表展示）
  productDescription?: string; // 商品描述（便于列表展示）
  productPrice?: number;     // 商品单价（便于列表展示）
  deliveryInfo?: string;     // 自动发货信息（买家下单后可见）
}
