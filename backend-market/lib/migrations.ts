...
async function ensureTaskColumns(db: DbClient) {
  if (!(await tableExists(db, 'tasks'))) return;
  const columns = await getTableColumns(db, 'tasks');

  if (!columns.has('submitted_at')) {
    await db.execute('ALTER TABLE tasks ADD COLUMN submitted_at INTEGER;');
  }

  if (!columns.has('contact_info')) {
    await db.execute('ALTER TABLE tasks ADD COLUMN contact_info TEXT;');
  }
}

async function ensureProductColumns(db: DbClient) {
  if (!(await tableExists(db, 'products'))) return;
  const columns = await getTableColumns(db, 'products');

  if (!columns.has('delivery_info')) {
    await db.execute('ALTER TABLE products ADD COLUMN delivery_info TEXT;');
  }
}

async function ensurePurchaseColumns(db: DbClient) {
  if (!(await tableExists(db, 'purchases'))) return;
  const columns = await getTableColumns(db, 'purchases');

  if (!columns.has('accepted_at')) {
    await db.execute('ALTER TABLE purchases ADD COLUMN accepted_at INTEGER;');
  }

  if (!columns.has('delivered_at')) {
    await db.execute('ALTER TABLE purchases ADD COLUMN delivered_at INTEGER;');
  }

  if (!columns.has('confirmed_at')) {
    await db.execute('ALTER TABLE purchases ADD COLUMN confirmed_at INTEGER;');
  }

  if (!columns.has('updated_at')) {
    await db.execute('ALTER TABLE purchases ADD COLUMN updated_at INTEGER;');
  }
}
...
export async function ensureSchemaForDatabase(db: DbClient): Promise<void> {
  ...
  ensurePromise = (async () => {
    ...
    await ensurePurchaseColumns(db);
