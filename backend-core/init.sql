...
CREATE INDEX idx_wallets_user_hash ON wallets(user_hash);
CREATE INDEX idx_wallets_last_active ON wallets(last_active_at);
...
CREATE INDEX idx_transactions_from_user ON transactions(from_user_hash);
CREATE INDEX idx_transactions_to_user ON transactions(to_user_hash);
...
CREATE INDEX idx_reports_tx_id ON reports(tx_id);
CREATE INDEX idx_reports_report_id ON reports(report_id);
CREATE INDEX idx_reports_reporter ON reports(reporter_user_hash);
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  status TEXT NOT NULL DEFAULT 'open',      -- 状态: open, in_progress, completed, cancelled
...
CREATE INDEX idx_tasks_creator ON tasks(creator_user_hash);
CREATE INDEX idx_tasks_acceptor ON tasks(acceptor_user_hash);
...
CREATE INDEX idx_products_seller ON products(seller_user_hash);
...
CREATE TABLE IF NOT EXISTS purchases (
  ...
  tx_id TEXT NOT NULL,                      -- 关联的交易ID
  status TEXT NOT NULL DEFAULT 'completed', -- 状态: completed, refunded
  created_at INTEGER NOT NULL DEFAULT (strftime('%s', 'now')),
...
CREATE INDEX idx_purchases_buyer ON purchases(buyer_user_hash);
CREATE INDEX idx_purchases_seller ON purchases(seller_user_hash);
CREATE INDEX idx_purchases_tx_id ON purchases(tx_id);
